...
export default function Page() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <nav className={`site-nav${isScrolled ? ' is-scrolled' : ''}`}><a href="#top" className="brand"><span className="brand-mark"><Sparkles className="size-4" /></span> O. I. SARAH<span className="brand-year">/ 2026</span></a><div className="nav-links"><a href="#details">Details</a><a href="#story">Moments</a><a href="/catalogue">Catalogue</a><a href="#rsvp" className="nav-cta">RSVP</a></div><button className="music-control" onClick={toggle} aria-label={playing ? 'Pause celebration music' : 'Play celebration music'}>{playing ? <Pause className="size-4" /> : <Music2 className="size-4" />}</button></nav>
      ...
      {rsvpOpen && <div className="modal-backdrop" role="presentation" onClick={closeRsvp}><section className="rsvp-modal" role="dialog" aria-modal="true" aria-labelledby="rsvp-title" onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={closeRsvp} aria-label="Close RSVP details"><X className="size-5" /></button><p className="eyebrow"><span className="eyebrow-dot" /> You are invited</p><h2 id="rsvp-title">Save<br /><em>your seat.</em></h2><p className="modal-intro">Kindly confirm your attendance by writing to Sarah Ogbang. We cannot wait to celebrate this beautiful milestone with you.</p><div className="account-card"><span className="account-label">GIFT / SUPPORT DETAILS</span><strong>Sarah Ogbang</strong><p>Zenith Bank · 1012345678</p><button className="copy-detail" onClick={() => navigator.clipboard?.writeText('Sarah Ogbang · Zenith Bank · 1012345678')}><Copy className="size-4" /> Copy details</button></div><a className="modal-mail" href="mailto:sarah.ogbang@example.com?subject=RSVP%20for%20the%20Graduation%20Celebration"><Mail className="size-4" /> Write to Sarah Ogbang <span>↗</span></a></section></div>}
    </main>
  )
}
